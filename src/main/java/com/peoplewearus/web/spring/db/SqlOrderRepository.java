...
import com.peoplewearus.web.spring.data.CouldNotUpdateOrderException;
...
public class SqlOrderRepository implements OrderRepository
{
	...
	private static final String INSERT_ORDER_STRING = "insert into order_table values (null, ?, ?)";
	...
	public SqlOrderRepository(ConnectionManager connectionManager, UserRepository userRepository,
			ProductRepository productRepository)
	{
	...
	private void addOrder(String email)
	{
		...
		try
		{
			...
			PreparedStatement insertOrderStatement = con.prepareStatement(INSERT_ORDER_STRING);

			insertOrderStatement.setString(1, email);
			insertOrderStatement.setString(2, "Ordered");
		...
		} catch (SQLException e)
		{
			...
			throw new RuntimeException("Could not add order", e);
		} finally
		{
	...
	public Order getOrder(String email, String state) throws OrderNotFoundException, ProductNotFoundException
	{
		...
		else
		{
			...
			try
			{
				PreparedStatement getOrderStatement = con.prepareStatement(GET_ORDER_BY_EMAIL_AND_STATE_STRING);
	...
	public Collection<Order> getAllOrders() throws ProductNotFoundException
	{
		...
		try
		{
			ResultSet rs = con
					.createStatement()
					.executeQuery(
							"SELECT * from product_table join orderline_table on product_table.product_id = orderline_table.product_id "
	...
	public List<Orderline> getAllOrderlines(int id) throws ProductNotFoundException
	{
		...
		try
		{
			...
			PreparedStatement getOrderStatement = con.prepareStatement(GET_ORDERLINES_BY_ORDER_ID);
	...
	private Orderline extractOrderline(final ResultSet resultSet) throws SQLException, ProductNotFoundException
	{
		return new Orderline(productRepository.getProduct(resultSet.getString("orderline_table.referenceNumber")),
				resultSet.getInt("orderline_table.amount"));
	}

	public Order updateOrderState(Order order, String state) throws CouldNotUpdateOrderException, OrderNotFoundException, ProductNotFoundException
	{
		...
		} else
		{
			...
			try
			{
				...
				PreparedStatement updateOrderStatement = con.prepareStatement(UPDATE_ORDER_STATE_STRING);
			...
			{
				throw new CouldNotUpdateOrderException();
			} finally
			{
				connectionManager.closeResource(con);
			}
	...
	private Order extractOrder(final ResultSet resultSet) throws SQLException, ProductNotFoundException
	{
